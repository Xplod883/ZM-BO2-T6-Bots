...
bot_get_explore_target()
{
    ...
    if(randomint(100) < 35)
    {
        ...
        if(isdefined(vending))
        {
            foreach(v in vending)
            {
                if(isdefined(v) && isdefined(v.origin))
                    poi_array[poi_array.size] = v.origin;
            }
        }
    ...
    if(isdefined(vending) && vending.size > 0)
    {
        ...
        if(findpath(self.origin, random_vending.origin, undefined, 0, 1))
        {
            visit = spawnstruct();
            visit.origin = random_vending.origin;
            visit.time = gettime();
            self.bot.visited_points[self.bot.visited_points.size] = visit;
            return random_vending.origin;
        }
    }
    return undefined;
}
