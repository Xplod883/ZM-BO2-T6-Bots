...
bot_tomb_register_objective_provider(func_ptr)
{
	if(!isdefined(level.tomb_bot_objective_providers))
		level.tomb_bot_objective_providers = [];
	
	level.tomb_bot_objective_providers[level.tomb_bot_objective_providers.size] = func_ptr;
}
...
bot_buy_perks()
{
    if(!isdefined(self.bot.perk_purchase_time) || gettime() > self.bot.perk_purchase_time)
    {
		...
		if(level.round_number >= 8)
		{
			...
			{
				self maps\mp\zombies\_zm_score::minus_to_player_score(best_cost);
				
				self thread maps\mp\zombies\_zm_perks::give_perk(best_perk);
			}
		}
	}
}
...
bot_get_explore_target()
{
    ...
    {
        if(gettime() - visit.time < 45000 && distancesquared(self.origin, visit.origin) < 4000000)
    ...
    for(attempt = 0; attempt < max_attempts; attempt++)
    {
        ...
        dist = randomfloatrange(1500, 5000);
        ...
        foreach(visit in self.bot.visited_points)
        {
            if(distancesquared(candidate, visit.origin) < 640000)
            {
                too_close = true;
                break;
            }
    ...
    {
        if(self.bot.visited_points.size >= 25)
        {
            trimmed = [];
...
bot_update_wander()
{
	...
	for(;;)
	{
		...
		if(isdefined(player))
		{
			...
			is_exploring = isdefined(self.bot.door_expansion_target) || (isdefined(self.bot.exploring) && self.bot.exploring);
			...
			if(self.bot.is_following)
			{
				if(!findpath(self.origin, player.origin, undefined, 0, 1))
				{
					...
					self.bot.follow_blocked = gettime() + 2000;
				}
				else
					self.bot.is_following = true;
		...
		
		{
			...
			if(!self hasgoal("wander") || self atgoal("wander") || time_at_point >= 2)
			{
				...
				{
					if(!isdefined(level.bot_explorers))
						level.bot_explorers = [];
					...
					foreach(entry in level.bot_explorers)
					{
						if(!isdefined(entry.bot) || !isalive(entry.bot))
							continue;
						...
						if(entry.bot == self)
						{
							...
							fresh_explorers[fresh_explorers.size] = entry;
						}
						else if(gettime() - entry.time < 20000)
						{
							fresh_explorers[fresh_explorers.size] = entry;
						}
					}
					
					level.bot_explorers = fresh_explorers;
					
					max_explorers = 3;
					
					if(!am_i_explorer && level.bot_explorers.size < max_explorers)
					{
						entry = spawnstruct();
						entry.bot = self;
						entry.time = gettime();
						
						level.bot_explorers[level.bot_explorers.size] = entry;
					...
					{
						location = get_random_walkable_location(self.origin, 1500, self);
					}
				}
