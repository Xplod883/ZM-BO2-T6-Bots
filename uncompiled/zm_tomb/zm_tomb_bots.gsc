...
bot_tomb_do_interact_shovel(stub)
{
    ...
    self thread bot_tomb_dig_cleanup_watcher(stub);
    ...
    if(isdefined(self) && isalive(self) && isdefined(stub) && isdefined(stub.e_shovel) && !is_true(self.dig_vars["has_shovel"])
    ...
    self notify("tomb_dig_attempt_done");
}

bot_tomb_dig_cleanup_watcher(claimable)
{
	self waittill_any("tomb_dig_attempt_done", "death", "disconnect");
...
bot_tomb_do_interact_dig_spot(target)
{
	...
	self thread bot_tomb_dig_cleanup_watcher(target);
